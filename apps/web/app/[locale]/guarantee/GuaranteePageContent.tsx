...
export default function GuaranteePageContent() {
  const t = useTranslations('guarantee');
  const locale = useLocale() as 'en' | 'zh';
  const { data: pageData } = usePageContent('guarantee');

  // Use database metadata if available, otherwise use i18n
  const heroTitle = pageData?.titleEn || t('title');
  const heroDescription = pageData?.metaEn || t('description');
      ...
      <section className="bg-primary text-white py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="text-3xl md:text-5xl font-bold mb-4">
            {locale === 'en' ? heroTitle : (pageData?.titleZh || t('title'))}
          </h1>
          <p className="text-xl text-warm-silver max-w-2xl mx-auto">
            {locale === 'en' ? heroDescription : (pageData?.metaZh || t('description'))}
          </p>
        </div>
      </section>
